...
void startedMessage(Message *msg, local_id id, pid_t pid, pid_t ppid, balance_t balance) {
    int size = sprintf(msg->s_payload, log_started_fmt, get_lamport_time(), id, pid, ppid, balance);
...
void doneMessage(Message *msg, local_id id, balance_t balance) {
    int size = sprintf(msg->s_payload, log_done_fmt, get_lamport_time(), id, balance);
    msg->s_header.s_magic = MESSAGE_MAGIC;
